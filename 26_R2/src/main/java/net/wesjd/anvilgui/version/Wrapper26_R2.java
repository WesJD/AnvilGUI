...
public final class Wrapper26_R2 implements VersionWrapper {
    ...
    private Class<?> inventoryCloseEventReasonClass;
    private Method paperHandleInventoryCloseEventMethod;
    ...
    public void handleInventoryCloseEvent(Player player) {
        try {
            ...
            try {
                if (inventoryCloseEventReasonClass == null) {
                    inventoryCloseEventReasonClass =
                            Class.forName("org.bukkit.event.inventory.InventoryCloseEvent$Reason");
                }
                if (paperHandleInventoryCloseEventMethod == null) {
                    paperHandleInventoryCloseEventMethod = Arrays.stream(CraftEventFactory.class.getMethods())
                            .filter(m -> m.getName().equals("handleInventoryCloseEvent"))
                            .findFirst()
                            .orElseThrow();
                }
                paperHandleInventoryCloseEventMethod.invoke(
                        null,
                        toNMS(player),
