...
public final class Wrapper1_21_R6 implements VersionWrapper {
    ...
    public void setActiveContainerDefault(Player player) {
        toNMS(player).cl = toNMS(player).ck; // cl -> containerMenu, ck -> inventoryMenu
    }

    @Override
    public void setActiveContainer(Player player, AnvilContainerWrapper container) {
        toNMS(player).cl = (Container) container; // cl -> containerMenu
    }
    ...
    private static class AnvilContainer extends ContainerAnvil implements AnvilContainerWrapper {
        public AnvilContainer(Player player, int containerId, IChatBaseComponent guiTitle) {
            super(
                    containerId,
                    ((CraftPlayer) player).getHandle().gB(), // gB -> getInventory
