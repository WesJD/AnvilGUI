 ...
 * @since 1.2.1
 */
public final class Wrapper1_13_R2 implements VersionWrapper {
    ...
    public int getNextContainerId(final Player player, final Object container) {
        return Wrapper1_13_R2.toNMS(player).nextContainerCounter();
    }

    @Override
    public void handleInventoryCloseEvent(final Player player) {
        CraftEventFactory.handleInventoryCloseEvent(Wrapper1_13_R2.toNMS(player));
    }

    @Override
    public void sendPacketOpenWindow(final Player player, final int containerId, final String guiTitle) {
        Wrapper1_13_R2.toNMS(player).playerConnection.sendPacket(new PacketPlayOutOpenWindow(containerId, "minecraft:anvil", new ChatMessage(Blocks.ANVIL.a() + ".name")));
    }

    @Override
    public void sendPacketCloseWindow(final Player player, final int containerId) {
        Wrapper1_13_R2.toNMS(player).playerConnection.sendPacket(new PacketPlayOutCloseWindow(containerId));
    }

    @Override
    public void setActiveContainerDefault(final Player player) {
        Wrapper1_13_R2.toNMS(player).activeContainer = Wrapper1_13_R2.toNMS(player).defaultContainer;
    }

    @Override
    public void setActiveContainer(final Player player, final Object container) {
        Wrapper1_13_R2.toNMS(player).activeContainer = (Container) container;
    }
    ...
    public void addActiveContainerSlotListener(final Object container, final Player player) {
        ((Container) container).addSlotListener(Wrapper1_13_R2.toNMS(player));
    }
    ...
    public Object newContainerAnvil(final Player player, final String guiTitle) {
        return new Wrapper1_13_R2.AnvilContainer(Wrapper1_13_R2.toNMS(player));
    }
